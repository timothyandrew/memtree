...
Load context from the local memtree that is relevant to the user's prompt. The `memtree` binary must be in PATH. Tree root defaults to `.memtree/` in the current working directory.
...
1. **Get the tree overview** — run `memtree ls --depth 2` to see the full structure with summaries.

2. **Search by keywords** — extract key terms from the user's prompt and run `memtree search <term>` for each. This finds leaves whose content or summary matches.
...
4. **Recall full content** — for each relevant path, run `memtree recall <path>` to load the full leaf content.
...
- If the tree is empty or `.memtree/` doesn't exist, tell the user and suggest using `/memtree-save` first.
